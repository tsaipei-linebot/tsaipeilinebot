...
81. **AI 需求單：沒有精準命中的句子交給 AI 判斷語意**（使用者 2026-09-23 決定：「沒辦法都想用程式判斷，要把沒有精準命中的給 AI 判斷語意，把邊界定義好」）。
    ...
    - **第三次考試**：單句 480/523（92%）；多輪 on 整段全對 180/203（off 139），變好 52 段、變差 11 段。剩下的錯誤共通點是 AI 會漏填或自己加條件、而且不穩定，所以加上**交叉檢查** `cross_check_form()`（在 `validate_form()` 裡，準確率考試也套用）：
      - AI 填的值要有根據：班別／休假／發薪／全兼職要在句子、沛沛上一句或目前條件裡找得到對應的字；排除要句子裡有否定詞；放寬要有「都可以／不限／隨便…」；薪資要有數字且講月薪或時薪（日薪不算）；福利「沒有X也沒關係」不是要X。
      - 程式認得但 AI 漏填的補上：地區（沒講住哪裡/從哪出發時）、類型（有講廠商時常漏）、休假、發薪、全兼職。
      - 否定詞緊貼著的（「白天不行」「夜班 不要」「不要台北」）以程式判斷的方向為準；以前的經歷（「以前做外場，現在想找工廠」）只看「現在想…」那段。
      - 簡稱對照（中市→台中、竹科→新竹…）；「做五休二」「收銀」加進程式詞庫。
      - AI 轉成標準句子後，不再對標準句子問「要還是不要」；只回「都可以／隨便」改回程式處理（`_is_program_phrase`）。
    - **第四次考試**：單句 476/523、多輪 on 182/203（單輪 95%），變好 54、變差 10。交叉檢查再補：否定方向也套用在休假／發薪／類型、「早班 不要假日班」的不要管後面那個、句子有講但 AI 排除的放回「要」；明確班別詞（小夜、日班、假日打工）跟薪資 AI 漏填時由程式補；「班別都可以／時間不限／其他縣市也可以／領錢方式都可以／不一定要日領／正職也行」由程式判定放寬；「改月領」是換掉；「我住龜山」的龜山不算；沒有對應類型的職務名稱裡的字不算類型；明講要找專員的關鍵字不讓 AI 推翻。拿上一輪 AI 答錯的 30 題重放，30 題都改對。
    - **第五次考試**：單句 483/523（AI 先想一下 `--thinking 512`：505/523，每句中位數 1.0 秒→1.8 秒）；多輪 on 179/203，變好 57、變差 15——變差是第四次加的交叉檢查造成的：程式把 AI 答對的否定句改錯（「高雄 作業員 不要輪班」變成不要作業員、「桃園 理貨 不要月領」變成不要桃園、「大夜班也不要／不想要輪班／可以不輪班嗎／不要去桃園」被放回「要」、「我住林口，上班地點林口或龜山」的林口被丟掉、「不用輪班」被當成放寬）。已修：「X 不要Y」的不要算在 Y 身上（後面沒話或接標點才算否定 X）、只有講到那個詞的那段話完全沒有否定詞才放回「要」、住的地方只有「每次提到都是住哪裡」才丟、班別的「不用／不需要」是不要。**教訓：交叉檢查只該擋「明顯沒根據」的值，不該推翻 AI 的判斷；之後不再往交叉檢查加規則。**
