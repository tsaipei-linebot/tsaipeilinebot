...
職缺維護比薪資補款複雜不少，還沒有開始細看以下這幾塊，動工前要先看懂：
- 「行業別」「職務類別」「全/兼職」「外籍生」「職缺週期」「縣市」
  「行政區」（縣市選了會連動篩選行政區）「班別」「休假方式」「領薪
  方式」這幾個多選下拉選單，選項是不是從 Notion 資料庫動態抓的、還是
  固定清單——要看 `Project_Job.gs`（尤其 `NotionService`／
  `AiJobDescriptionService` 一類的邏輯）才能確定。
- 「新增全新職缺」跟「維護既有職缺」兩種模式：後者要能搜尋、篩選出
  「自己刊登的」或「轄下同仁刊登的」既有職缺（用刊登人身分判斷可不
  可以改），跟現有帳號的 `manager_usernames` 主管關係要怎麼接起來。
- 圖檔上傳（O 欄位圖檔，跟薪資補款佐證照片是類似的上傳機制，可以參考
  同一套做法）。

這是目前排定的下一項工作，範圍比薪資補款大很多，還沒有開始動工。
