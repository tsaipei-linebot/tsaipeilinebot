...
- **到期提醒**：改成**每週一上午 10 點**檢查一次，到期前 30 天內或已過期、還沒更新日期
  的全部推到群組，同仁更新日期後就不再提醒（原本每 7 天最多一次的「重送間隔」邏輯可以
  拿掉）。第一次通知落在到期前 24～30 天之間的那個週一，使用者同意。Cloud Scheduler
  的 `delivery-expiry-reminder` 要從 `0 9 * * *` 改成 `0 10 * * 1`（`gcloud scheduler
  jobs update http delivery-expiry-reminder --schedule="0 10 * * 1" ...`，實作完給
  使用者指令）。提醒要排除「放棄報到」「離職」的人。
