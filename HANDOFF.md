...
- **【已定案設計，尚未實作，等切換正式頻道才啟用】監控與告警機制**：取代原本「考慮加上錯誤告警機制」的初步想法，已經跟使用者討論定案完整設計：
  - 底層數據來源：結構化 log → Cloud Monitoring 指標（方案 A，非直接串接 log-based alert）
  - 機制數量：只有一個——每日排程檢查，不做即時告警
  - 檢查頻率：每天一次
  - 內容：整體健康狀況＋過去 24 小時是否觸發第一層／第二層異常
  - 第一層門檻（明確故障）：保底訊息出現 1 次就算異常
  - 第二層門檻（明顯變慢）：p95 延遲超過 12 秒、持續 3 分鐘以上
  - 通知管道：這個 Claude 對話串＋LINE 群組，兩邊都發
  - 啟用時機：等切換到正式頻道才生效，測試期間不跑
  - **要落地時需要做的事**（不急，等使用者確定要開始實作才動工）：① 在 AI 決策那段程式碼加結構化 log；② 設定 Cloud Monitoring 的記錄型指標；③ 設定每日排程喚醒（Claude Code 這邊的排程機制）；④ 使用者需要先拉好要發通知的 LINE 群組，並把沛沛（官方帳號）加進去，方便之後取得群組 ID

- **【討論中，尚未實作】每週彙整求職者常問內容以擴充 FAQ**：使用者提議定期把「求職者常問的內容」整理成新的 FAQ，藉此加快回覆速度、降低呼叫 Gemini 的次數。討論後拆成兩條本質不同的線，處理方式不一樣：
  - **第一條線（真正適合變 FAQ 的）**：答案不會隨時間改變的政策類問題（發薪日、休假規範、需準備文件、投保方式等）。系統目前已經有現成的原始資料——AI 判斷成 `UNKNOWN_FAQ`（沒收錄的政策類問題）時，會自動把原始問句寫進 Notion FAQ 資料庫。規劃是每週排程用 Gemini 讀這些累積問句，做語意分群＋排序，列出「這週最常被問、還沒有答案」的候選清單＋建議答案草稿。**關鍵原則：絕對不能讓 AI 生成的答案自動上架**，一定要人工（使用者或負責 FAQ 的同事）審核、微調過再手動填進 Notion，否則會失去 FAQ 機制原本要避免的合規風險（規章／福利類文字寫錯有法律風險）。
  - **第二條線（職缺類問句，不能變 FAQ）**：像「有新莊的工作嗎」「有 momo 的工作嗎」這種問題，正確答案每天都在變（職缺數量、是否還在招），寫死成 FAQ 直接回會有給出過期職缺資訊的風險，比現在多花幾秒查即時資料更危險，**不建議這樣做**。這條線該做的是定期檢視這些常見問句有沒有被現有的「直接分類快速路徑」（`matcher_service.py` 的 `CATEGORY_KEYWORDS`／`KNOWN_BRANDS` 關鍵字清單）接住，例如發現大家常用「宅配」而不是「外送」但關鍵字清單沒收錄，就補進去——一樣能做到免呼叫 AI、秒回，但因為還是即時查 Notion 現有職缺資料，不會有過期風險。
  - **通知整合**：這兩條線的每週整理結果，規劃直接掛在上面「監控與告警機制」的每日／每週報告上，不用另外做一套通知管道——例如每週固定加一段「FAQ 候選清單」＋「建議新增的職缺關鍵字」，一樣發到 Claude 對話串跟 LINE 群組審閱。
  - **待確認細節（已部分確認）**：FAQ 候選清單由使用者請同仁負責整理／審核，不是使用者自己一個人拍板。
  - **初期策略：先求量、不急著在寫入端過濾雜訊**：實際用 Notion 唯讀連線檢查過 `材霈_客服常見問答庫`，目前有 12 筆「標準回覆內容」空白的候選問句，內容有明顯的政策類問題（如「加班費怎麼計算？」「颱風天上班算加班嗎」），也有看起來像誤判進來的職缺類問句（如「有不用面試的工作嗎？」，本質是職缺篩選條件不是政策問題）、以及意圖不明的雜訊（如「照片」「有樣子嗎」）。使用者確認：求職者問法本來就會比較混亂，**初期先維持現有邏輯全部寫入 Notion，不需要在寫入端先過濾雜訊**，希望先大量累積 FAQ 候選內容，之後交給同仁去整理／篩選／回答。之前討論的「過濾雜訊」「關鍵字缺口偵測」這類精緻化調整，等有實際週報機制、同仁整理出使用心得之後再視情況調整優先度，不是現在的重點。
