...
薪資補款送出表單、職缺維護都已經完成並經使用者實測確認。「職缺維護
表單整合案」排定的下一項是「專案合約維護」（廠商/合作類別/簽約模式/
約訪專員/拜訪主管 + 合約檔案上傳，沒有核准流程，是三個裡最簡單的一個，
原始碼已經有了，見 `ProjectWorkflowService.gs`），之後是「LINE 登記
流程重建」（跟「方案 B：薪資補款核准流程整段搬進來」綁在一起做，目前
還沒排時間）。

動工「專案合約維護」之前，跟前兩個表單一樣的老規矩：跟使用者要一次
真正的前端原始碼（HTML/JS，不是只有截圖或 `.gs` 後端程式碼），欄位
名稱、必填規則、選項清單才能真的做到「完全比照」，不要用猜的。
