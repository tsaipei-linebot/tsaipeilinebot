...
**還有兩個欄位的下拉選單選項，只從畫面截圖看得到目前選取的那一個值，
完整選項清單還沒跟使用者確認**（`services/salary_repayment_submit_service.py`
裡的 `IS_CLAIMABLE_OPTIONS`/`PAY_TYPE_OPTIONS`，程式碼註解裡也有標註）：
- 「是否可請款」：截圖看得到「可」，先假設另一個選項是「不可」（先當
  推測值使用，需要使用者比對現有表單確認是否正確）。
- 「補款方式」：截圖看得到「立即補款」，還不知道下拉選單裡還有哪些
  其他選項，目前只放這一個，需要使用者提供完整清單才能補齊——同仁
  打開這個欄位目前只會看到一個選項可選。
