...
## 【待辦，規格大致確認、等使用者說「請實作」】人資代傳各所加退保 ＋ 收單後補件 ＋ 新增「蝦皮」部門（2026-09-23 提出，2026-09-25 確認規格）
...
- 因為有補件機制，「收單後下載待送出清單交給人資」這條路之後可以拿掉或保留當備用（實作時再問）。

**蝦皮（需求 2）**
- 「部門/店家」填**主要門市**（Q4 選 A）；「廠商」統一填「**蝦皮門市**」（Q5）。
- Q6（備註帶課程類型）、Q7（離店日期當退保日）、Q8（已離職照樣納入）、Q9（彙總頁多一列蝦皮）：
  **建議都是「要」，等使用者確認**。
...
3. 勾選工廠 →「寄出介紹信」→ 預覽 → 寄出。範本可在平台修改，變數 `{公司名稱}`、`{工廠地區}`
   （職缺名單版開頭改用 `{職缺名稱}`）。一天上限預設 20 封、同一 Email 不重寄、「個人」類型
   不寄、兩個 Email 只寄第一個；每封記錄誰/何時/寄給誰/成功失敗。
