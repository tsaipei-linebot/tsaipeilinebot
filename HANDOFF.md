...
**第一次正式執行（2026-09-26 10:24）**：API 回傳 18,725 筆、符合關鍵字 3,904 筆職缺、2,257 間公司，92 個
郵遞區號一次就抓完；剩下的時間讀了 32 個職缺頁、17 個有 Email（53%）。**Cloud Run 連得到、HTTPS 憑證驗得過**。
使用者把排程改成每 30 分鐘（`--schedule="*/30 * * * *"`）加快讀完 3,872 頁。

**被擋保護（2026-09-26 第二個 PR）**：原本一頁失敗就算一次、3 次放棄，如果台灣就業通暫時擋我們，會有一大批
職缺被標成「讀不到」再也不讀。改成 `taiwanjobs_pipeline.BLOCK_STREAK`（5）：連續 5 頁失敗就停止這次執行、
這幾頁**不算**失敗次數、畫面紅字提醒；中間有讀成功的頁面才把前面失敗的照常算次數。之後要改回每 15 分鐘或
更頻繁，先看幾次執行有沒有出現這個紅字。
...
**每封信只有一個收件人**（同公司多個信箱各自一封）；內文跟簡介各有好幾個版本；簡介是文字＋PDF，PDF 自己夾；
...
內文使用者還要再確認，先放一份「暫用範本／暫用簡介」。

- 列表（`/salesdev?tab=taiwanjobs`）：最左「寄信」（沒有 Email 時是灰的）＋「編輯」，最右「發信紀錄」（最近日期、
  共幾次，滑鼠移上去看明細）；多一組篩選「還沒寄過／寄過」、「信件範本」入口。Email/電話欄改顯示
  `effective_emails()`／`effective_phones()`。
- 公司頁 `/salesdev/taiwanjobs/companies/{EMPLOYER_ID}`（`templates/salesdev_tj_company.html`）：選內文範本＋簡介版本
  （下拉選單，選了會記在 cookie，下次預設同一組）→ 預覽主旨/內文（可以直接改，只影響這一次）→ 每個 Email 一顆
...
- `salesdev/mail_templates.py`＋`templates/salesdev_templates.html`（`/salesdev/templates`）：Firestore
  `salesdev_mail_templates`，`kind` body（名稱＋主旨＋內容）/intro（名稱＋內容＋附件檔名），可新增、修改、停用；
  變數 `{公司名稱}{聯絡人}{職缺名稱}{地區}{簡介}`，沒資料時聯絡人寫「人資負責人」、職缺寫「產線人員」；
  一個範本都沒有時自動放暫用內文/簡介。
- 「下載 Excel」的台灣就業通工作表多「最近寄信日期」「寄信次數」，Email/電話改成含手動、扣掉隱藏的。
- 測試 `tests/test_salesdev_mail.py`（15 個）；用 Playwright 實際點「開啟 Gmail」確認網址（單一收件人、用畫面上改過的主旨）。
...
### 使用者的決定（2026-09-26，**使用者要求「討論好再啟動」，還沒動工**）

1. 員工主管組織表 → **搬進平台帳號**（主管用 `manager_usernames`，補款也改讀平台帳號）。
2. 已綁定的人 → **從組織表一次匯入平台帳號**（姓名比對，對不上的列清單給使用者手動處理，確認後才寫入）；
   **新進同仁改用驗證碼綁定**（我的專區按「綁定 LINE」拿驗證碼傳給官方帳號），PIN 廢掉。
3. Netlify 舊表單 → 可以關，等最後一個 PR 上線、大家都改用平台表單後再給使用者關閉步驟。
4. AI 文案 → **要另外調教**（先做預覽比對頁）。討論時發現的問題一起處理：**「修改職缺」會越改越失真**——送出時 AI
   改寫的「對外工作內容」直接蓋掉同仁原文存進 Notion，下次修改表單帶出的是 AI 版、再送一次 AI 又改寫一次。
   搬家時另外保留同仁原文、每次都從原文產生。（批次優化本身不會：它讀對外內容、只寫精華亮點／排版說明，
   續跑模式還會跳過已有精華亮點的。）
5. 排程 → GAS「觸發條件」只有一個：`checkAndUpdateJobStatuses`，每天上午 2 點。**但程式裡根本沒有這個函式**
   （實際入口叫 `CheckJobStatuses`，`Project_Job.js` 第 7 節），使用者到「執行項目」確認**每天都失敗**——所以
   「一週內有更新」滿 7 天改「招募中」一直沒有在跑。使用者選**先不修 GAS，搬家時一起處理**（平台排程每天 2 點，
   第一次跑會把卡住的職缺一次改成招募中）。批次優化**沒有計時器**，一直是手動跑 → 搬家後在管理頁放按鈕手動執行。

### AI 文案調教的決定（2026-09-26）

- **過濾規則（`enforceComplianceRules()`）先不改**，照搬（→ 同一天後來使用者同意修掉誤刪和漏網，見下面「階段 3 第 1 個 PR」）。討論時提過但使用者決定不動的：「健全」整個刪會把
  「福利制度健全」刪成「福利制度」；年齡過濾會連「須年滿 18 歲」一起刪。
- GAS 送出／批次優化其實**已經用 Vertex AI（gemini-2.5-flash → flash-lite）**，跟平台同一種，照搬 prompt 產出應該差不多。
- **「排版工作說明」統一成一種格式**（現在送出是 6 區塊版、批次優化是另一種精簡版，Notion 裡兩種混在一起）：
  以**送出版（6 區塊：📋職缺名稱／🎯主要工作內容／⏰工作時間與休假／💰薪資與福利待遇／📍工作地點與交通／💡就業服務法聲明）
  為主，加上批次版的「✨應徵與配合條件」**，送出和批次優化都用這一種；舊職缺之後可用批次優化強制模式統一。
- **「✨應徵與配合條件」**：確認過批次版是 AI 從 Notion「工作內容(對外)」（已經是送出時 AI 改寫過的版本）整理的，程式
  沒有事後檢查。改成**從同仁原文抓**（平台另存原文），**原文找不到條件就整個區塊不出現**，不讓 AI 硬湊；核准卡片上
  顯示給主管看。
- 批次優化的 prompt 補上送出版才有的「客戶去識別化」（台積電→知名半導體大廠）。
- **語氣**（GAS 原本沒有明確語氣指示，只靠角色「資深勞動法規顧問與專業人資文案專家」＋「吸睛標題」「吸引短句」帶出
  「標題亮點活潑、內文專業」）：**整體改得更親切**；表情符號維持現在的量；**稱呼固定用「您」**；「高薪」「爆缺」這類字
  **不禁止**；**不加固定結尾的報名句**（文案會連動到官網和沛沛聊天機器人，兩邊都已經有報名機制）。
- **防腦補要做到「程式檢查」而不是只靠 prompt**（使用者要求 100% 防止 AI 腦補；跟使用者說明過 AI 本身做不到保證，
  所以用程式把關＋主管核准兩道）：AI 產出後比對——(1) 產出裡的數字（薪資、時段、公斤數、天數…）都要出現在同仁原文／
  欄位裡；(2) 常見福利／條件關鍵字（無經驗可、週休二日、供餐、交通車、獎金、加班費、固定班…）產出有、原文沒有就算腦補；
  不過關就重新產生一次，再不過就用只寫事實的保底文案，並在核准卡片上標「AI 文案未通過檢查，請人工確認」。
...
- **`services/job_copy.py`（新檔）**：
  - 照搬：`format_smart_location()`、`enforce_compliance_rules()`（過濾規則修過誤刪／漏網，見下面）、4 樣文案、保底文案。
...
- **`/job-listings/migration`（只有全平台管理員，`job_copy_preview_routes.py`）**：職缺清單 → 「預覽」左右對照
  ...
  職缺維護頁面管理員看得到連結。
- **修掉 GAS 過濾規則的誤刪和漏網**（使用者 2026-09-26 同意；原則：**違反就業服務法的字一定要刪，分不清楚時從嚴刪**）：
  - 誤刪的原因：GAS 3(B) 那條「限／須／需＋數字範圍」的排除條件會被正規表示式回溯繞過——「需 8-17點」變「7點」、
    「須 10-20公斤」變「0公斤」、「需 20-30 分鐘」變「0 分鐘」；排除清單又沒有年、個月、天、人，「需 2-3 年經驗」
    被刪成「年經驗」。改法：數字要整段吃完（`_NUM` 後面不能再接數字）；範圍後面接單位（`_NOT_AGE_UNITS`：點、分、公斤、
    年、個月、天、人、元、K…）就保留；有「歲」或前面寫「年齡／年紀」一律刪；什麼單位都沒寫的「限 18-35」兩個數字都在
    15～70（`_AGE_MIN/_AGE_MAX`）就當年齡刪，國字數字判斷不了也刪。
  - 殘字：「限男性」原本剩「性」、「限男生搬重」剩「生搬重」（換句的規則排在刪「限男」後面永遠跑不到）、「需18-35歲」
    剩「需」→ 換句規則移到前面、長的寫法排前面、連前面的「需／限／年滿」一起刪。
  - 漏網補上：「男性優先」「女生佳」、婚姻（限未婚、已婚者優先）、籍貫／出生地（限台北人、桃園人優先，`_PLACES` 列縣市）、
    星座、血型。刻意**不動**「限本國籍」（外籍另有工作許可規定）、「限本人親自領取」。
  - 誤傷用字：「健全」只刪「身心／四肢／身體／心智健全」（「福利制度健全」保留）；「年輕化團隊」→「充滿活力的團隊」、
    「年輕有活力」→「充滿活力」。「須年滿 18 歲」照舊刪（之前討論的決定）。
  - **GAS 正式環境沒修**：使用者選等平台版（第 3 個 PR 切換後就由平台產文案），GAS 在那之前照舊有這個誤刪。
  - `tests/test_job_copy.py::ComplianceTests` 列出每一種違法寫法都要刪乾淨（空字串、不留殘字），以及每一種正常的時間／
    重量／天數寫法都要原樣保留；以後改規則任何一條違法字眼沒刪到，測試就會失敗。
- 測試 `tests/test_job_copy.py`（19 個）。
