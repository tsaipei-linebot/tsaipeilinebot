...
2. **薪資補款**（提交端）：同仁在這裡送出補款申請，寫進一份 Google
   ...
   搬過來。
3. **專案合約維護**：❓ **完全不知道實際內容，需要使用者介紹或提供
   原始碼才能評估**——這是接下來要問清楚的第一件事。
...
**還沒決定的關鍵問題**：整合後身份驗證要不要一併換成 `platform_accounts`
這套帳號密碼系統？
- 換成這裡的帳號系統：跟「所屬主管」「部門」這些已經做好的欄位一致，
  之後任何新功能都可以直接用同一套身份，但需要幫還沒有平台帳號的
  同仁一一建帳號，是額外的導入成本。
- 繼續沿用姓名+PIN：同仁操作習慣不變，但這套身份系統會繼續跟平台帳號
  系統脫節。

### 建議的分階段順序（尚未開始執行任何一步）

1. 請使用者介紹「專案合約維護」的實際內容、或提供職缺維護表單的原始碼
   / 畫面截圖，把功能全貌看清楚。
2. 決定身份驗證要不要換成 `platform_accounts`。
3. 動工順序建議：**薪資補款送出表單**（跟現有讀取端銜接，風險最低）→
   **職缺維護**（延伸現有 Notion 寫入模式）→ **專案合約維護**與
   **LINE 登記流程重建**（要看到細節才能評估，風險相對最高，建議放
   最後）。

### 下一步

等使用者提供「專案合約維護」的細節或原始碼，才能往下規劃具體的資料
模型跟頁面設計。
