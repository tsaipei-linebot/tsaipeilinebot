...
- **【新功能，程式碼已在功能分支上寫完並跑過測試，但刻意還沒合併進 main、也還沒部署】面試預約：求職者填完履歷後可以直接在 LINE 對話裡選面試時段**：使用者提出「人員填完履歷後，讓人員選擇面試時間」的需求，討論後定案的流程是：
  1. 職缺卡片新增一顆「📅 預約面試」按鈕（跟「📄 填寫線上履歷」並列，不是點了履歷連結之後才出現——LINE 的 `uri` 類型按鈕點下去是開外部瀏覽器，機器人完全收不到「使用者填完履歷、返回聊天室」這個事件，沒辦法做成「先填完才能選時段」的接續流程，只能兩個選項同時並列）。
  2. 按下「📅 預約面試」後，沛沛會先問一次「請問是否已經填寫過線上履歷了呢？」（Yes/No 快速回覆），選「還沒」的話重新附上履歷連結、鼓勵填完再回來，不會直接跳進時段選單。
  3. 選「已經填好了」才會去讀 Notion「面試時段」資料庫，把還沒額滿、還沒過期的開放時段列成快速回覆按鈕給使用者選（最多列 10 個，避免超過 LINE 快速回覆最多 13 顆按鈕的上限）。
  4. 使用者選定時段後，沛沛會寫入 Notion「面試預約」資料庫（含求職者暱稱、LINE User ID、應徵職缺、面試時間），並把該時段的「已預約人數」+1，讓招募專員能在 Notion 後台看到誰預約了哪個時段。如果選的時候剛好被別人搶滿了（極低機率但有考慮到的併發情境），會老實告知並重新列出還有的時段。
  - **這兩個 Notion 資料庫都還沒建、程式碼也還沒合併部署，這項功能目前完全不會生效**——`config.py` 的 `NOTION_INTERVIEW_SLOTS_DB_ID`／`NOTION_INTERVIEW_BOOKINGS_DB_ID` 只要有一個沒設定，相關函式會直接回傳空清單/安全跳過，求職者按「📅 預約面試」按鈕還是能收到「目前尚未開放預約」這類老實的回覆，不會整個機器人壞掉或回覆奇怪的錯誤——但也代表現在合併部署了也不會有任何實際效果，等使用者確認要開始使用時，再做以下事情：
    1. **在 Notion 建立「面試時段」資料庫**（同仁自己維護，決定哪些時段開放、每個時段上限收幾人），欄位需要：
       - `時段名稱`（標題欄位，例如「9/15 早上場」，純粹給同仁自己辨識用，機器人不會直接顯示這個名稱給求職者，會用下面的「面試時間」自動組出「9/15(一) 14:00」這種格式）
       - `面試時間`（日期類型，可以只填日期、或連時間一起填）
       - `可預約人數`（數字類型，這個時段最多能收幾人）
       - `已預約人數`（數字類型，一開始都填 0，機器人每成功預約一筆會自動 +1，同仁不用手動維護）
       - `狀態`（單選類型，選項至少要有「開放」——沒有這個欄位或沒選「開放」的時段，機器人不會列出來給求職者選；同仁想暫停某個時段開放，把狀態改成別的選項即可，不用整筆刪除）
    2. **在 Notion 建立「面試預約」資料庫**（機器人寫入用，同仁不用自己填），欄位需要：
       - `求職者暱稱`（標題欄位，機器人會自動填 LINE 顯示名稱）
       - `LINE User ID`（文字類型）
       - `應徵職缺`（文字類型）
       - `面試時間`（日期類型）
       - `已回覆`（勾選類型，機器人寫入時都是未勾選，同仁聯繫過求職者確認後可以手動勾起來，方便追蹤還有哪些人沒聯繫到）
    3. **把這兩個資料庫都分享給 Notion 整合（integration）**：跟其他資料庫（例如「求職者提問追蹤」）一樣的做法——打開資料庫頁面右上角「⋯」→「連結」（Connections）→ 找到目前用的那個整合名稱加進去，不然機器人會讀不到/寫不進去。
    4. **取得兩個資料庫的 ID**：打開資料庫頁面，網址列 `https://www.notion.so/xxxxxxxx?v=yyyyyyyy` 裡面 `?v=` 前面那一串 32 位英數字（可能中間有 `-`，去掉 `-` 或保留都可以）就是資料庫 ID，「面試時段」「面試預約」各記一組。
    5. **到 Cloud Run 設定這兩個環境變數**（Cloud Shell 執行，`YOUR_SLOTS_DB_ID`／`YOUR_BOOKINGS_DB_ID` 換成上一步取得的實際 ID）：
       ```bash
       gcloud run services update recruitment-bot \
         --project=tsaipei-505807 --region=asia-east1 \
         --update-env-vars="NOTION_INTERVIEW_SLOTS_DB_ID=YOUR_SLOTS_DB_ID,NOTION_INTERVIEW_BOOKINGS_DB_ID=YOUR_BOOKINGS_DB_ID"
       ```
       設定完 Cloud Run 會自動重新部署一次新版本（不用另外重啟），等個一兩分鐘即可。
    6. **確認設定成功**：在「面試時段」資料庫裡新增至少一筆「狀態」選「開放」、「面試時間」是未來日期的時段，接著在 LINE 測試頻道對沛沛說「都給我看看」拿到一張職缺卡片，按下「📅 預約面試」→「✅ 已經填好了」，應該就能看到剛才新增的時段出現在選單裡；選一個時段送出後，回頭去「面試預約」資料庫確認有沒有新增一筆對應紀錄、「面試時段」裡對應時段的「已預約人數」有沒有 +1。
  - 這個功能目前只存在於 `claude/tsaipei-linebot-handoff-7jdsks` 這個分支上，**還沒合併進 main、也還沒部署到 Cloud Run**，等使用者確認要上線（並準備好上面兩個 Notion 資料庫）之後再合併部署。

- **【新功能，程式碼已在功能分支上寫完並跑過測試，但還沒合併進 main、也還沒部署】履歷點擊紀錄：記錄誰點了職缺卡片的「填寫線上履歷」按鈕**：使用者希望知道有點擊這顆按鈕的求職者 LINE 名稱，方便招募專員追蹤。技術上有個關鍵限制：這顆按鈕是 LINE 的 `uri` 類型（點下去直接開外部瀏覽器），**點擊本身完全不會觸發任何 webhook 事件**，機器人原本沒有任何方式能在伺服器端知道誰點了它。
  - **做法**：讓按鈕改連到我們自己這支服務新增的 `/apply-click` 轉址端點（`main.py`），把「誰、點了哪個職缺、哪個產業分類」記錄進新的 Notion「履歷點擊紀錄」資料庫，再用 HTTP 302 立刻轉址到真正的履歷網站——求職者感覺不出差異，只多花幾乎瞬間的伺服器轉址時間。
  ...
  - **兩個新設定值只要有一個沒設，功能安全跳過**（按鈕維持原本直接連到履歷網站的行為，不會壞掉）：
    1. **在 Notion 建立「履歷點擊紀錄」資料庫**（機器人寫入用，同仁不用自己填），欄位需要：
       - `求職者暱稱`（標題欄位，機器人會自動填 LINE 顯示名稱，查不到時退回顯示 LINE User ID）
       - `LINE User ID`（文字類型）
       - `應徵職缺`（文字類型）
       - `產業類別`（文字類型，機器人會自動填「蝦皮/外送」「門市/服務餐飲」「廠務/其他」其中一種）
       - `點擊時間`（日期類型）
    2. **把這個資料庫分享給 Notion 整合**：打開資料庫頁面右上角「⋯」→「連結」（Connections）→ 找到目前用的那個整合名稱加進去。
    3. **取得資料庫 ID**：打開資料庫頁面，網址列 `https://www.notion.so/xxxxxxxx?v=yyyyyyyy` 裡面 `?v=` 前面那一串就是資料庫 ID。
    4. **到 Cloud Run 設定兩個環境變數**（Cloud Shell 執行，`YOUR_CLICK_LOG_DB_ID` 換成上一步取得的 ID；`SERVICE_BASE_URL` 這個repo目前實際的 Cloud Run 網址已經是 `https://recruitment-bot-412901869672.asia-east1.run.app`，如果之後服務網址有變動要記得跟著改）：
       ```bash
       gcloud run services update recruitment-bot \
         --project=tsaipei-505807 --region=asia-east1 \
         --update-env-vars="NOTION_RESUME_CLICK_LOG_DB_ID=YOUR_CLICK_LOG_DB_ID,SERVICE_BASE_URL=https://recruitment-bot-412901869672.asia-east1.run.app"
       ```
       設定完 Cloud Run 會自動重新部署一次新版本，等個一兩分鐘即可。
    5. **確認設定成功**：在 LINE 測試頻道跟沛沛要一張職缺卡片，點下「填寫線上履歷」按鈕，應該還是正常導去履歷網站（求職者完全感覺不出差異），接著回頭去「履歷點擊紀錄」資料庫確認有沒有新增一筆對應紀錄（求職者暱稱、職缺、時間都對）。
  - 這個功能目前只存在於 `claude/tsaipei-linebot-handoff-7jdsks` 這個分支上，**還沒合併進 main、也還沒部署到 Cloud Run**，等使用者確認要上線（並準備好上面的 Notion 資料庫與環境變數）之後再合併部署。
...
47. **接續發現：職缺涵蓋 5 個以上行政區、地點欄位改用「各區OO據點（自選區域）」概括描述時，AI 還是會自己腦補聲稱某個行政區也算涵蓋在內，甚至跟自己剛講過的話矛盾**：使用者實測「蝦皮門市」→「八德」（直接攔截誠實回答「八德目前沒有明確列出的蝦皮門市職缺」，並附上同縣市退讓建議卡片，這部分正確）→「有哪些區？」，這句話沒有指名任何行政區、也不是「都給我看看」這類既有攔截關鍵字，直接落到 AI 決策，AI 卻回覆「沛沛為您找到蝦皮門市在桃園市各區都有據點可選喔！八德區也在可選範圍內，歡迎參考！」——直接打臉自己上一句話才剛說「八德沒有明確列出」。
