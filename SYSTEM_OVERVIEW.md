...
| 合約產生器 | `/client-contracts` | 需 `/accounts` 開通 | 填入客戶公司資料、合約期間與費率，自動套版產生企業服務合約 Word 檔並存檔，可帶入專案合約維護送出 |
...
18. 到 `/companies` 補上每家公司的「勞工保險證號」「勞退提繳單位編號」
    （這次建立時還沒有這兩項資料）。
22. 到 `/companies` 補上每家公司的「地址」（2026-09-12 新增的欄位，
    給合約產生器的乙方欄位帶入用，之前建立的公司都還沒有這項資料）。
...
（可見範圍 2026-09-11 已收斂：原本任何有這個模組權限的帳號都看得到全部
同仁的紀錄，現在只有送出者本人、送出者的主管、或平台管理員看得到，列表
頁、下載、預覽都受這個限制，詳見 `HANDOFF.md`。）

**合約產生器（`/client-contracts`）上線前還缺：**
23. 到 `/companies` 幫現有 10 家公司補上「地址」（跟上面第 22 點同一件
    事，這個功能的乙方欄位需要用到）。
24. 到 `/accounts` 幫需要用這個功能的帳號開通「合約產生器」權限（不需要
    新的環境變數，沿用既有的 `DELIVERY_GCS_BUCKET`）。
25. 上線後找一個已知統編/名稱的客戶實際測試一次「甲方查詢」，確認外部
    公司登記資料查詢功能正常運作（查不到的話會自動退回手動輸入，不影響
    合約產生本身）。
26. 測試「從合約產生器帶入」跟 `/project-contracts` 的串接：先產生一份
    測試合約，再到 `/project-contracts` 確認選單看得到、選了之後欄位跟
    檔案都正確帶入，送出後回合約產生器列表頁確認顯示「已送出」。

（2026-09-12 新增「實支實付」第二個合約版本、費率表拿掉「元/hr」單位
文字、檔名/存檔名稱加上合約起始年份、列表頁新增「複製」功能給續約用，
詳見 `HANDOFF.md`。甲方查詢原本想再加「台灣公司網」，查證後發現沒有
公開 API、只能用網頁爬蟲，使用者確認後選擇不加，維持現有兩個來源。）
