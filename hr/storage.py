"""上傳/下載體檢報告、關懷紀錄附件、證照、教育訓練證明用的 GCS 存取層。跟
delivery/storage.py、management/storage.py 是同一套做法（私有 bucket，一律
透過需要登入 session 的下載路由讀取，不開放公開網址或簽名連結），共用同一個
bucket，只是 blob 路徑前綴改成 hr/，避免跟其他模組的檔案混在一起。
"""
import uuid

from config import GCP_PROJECT_ID
from hr.config import GCS_BUCKET_NAME
...
def upload_file(category: str, entity_id: str, filename: str, content: bytes, content_type: str) -> str:
    ...
    blob_path = f"hr/{category}/{entity_id}/{uuid.uuid4().hex}{ext}"
...
def download_file(blob_path: str):
    ...
    if not blob_path.startswith("hr/"):
        return None, None
    blob = _bucket().blob(blob_path)
    if not blob.exists():
        return None, None
