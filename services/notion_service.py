...
def query_notion_database_direct(database_id: str) -> list:
    """以原生 HTTP POST 請求 Notion 資料庫，支援分頁讀取[cite: 2]。"""
...
def fetch_jobs_data() -> list:
    """取得招募中職缺資料（具備 30 秒快取機制與結構化欄位補齊）[cite: 2]。"""
    ...
    try:
        ...
        for page in results:
            ...
            # 3. 讀取其餘白名單屬性 (含 精華亮點 與 排版工作說明)[cite: 2]
