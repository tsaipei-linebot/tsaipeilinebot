    ...
    with _faqs_cache_lock:
        ...
        try:
            ...
            for page in results:
                ...
                # 改成「白名單」判斷（只有明確等於「啟用」才會被沛沛拿來回覆），
                # 取代原本的「黑名單」判斷（只要不是「停用」/「關閉」/「false」
                # 就算數）。使用者要求要能明確控制：問答內容跟答案都填好了，
                # 也要同仁自己把「啟用狀態」改成「啟用」才會真的生效，不能只
                # 靠「沒有主動停用」就自動上線——避免同仁在 Notion 上編輯中、
                # 還沒審完的草稿被誤當成正式答案送給求職者。
                if status == "啟用" and q_text and a_text:
...
def fetch_pending_faq_candidates() -> list:
    ...
    「啟用狀態」欄位在這裡是重複使用、不是另外新增專用欄位：
      ...
      - 填了「標準回覆內容」、且「啟用狀態」也手動改成「啟用」= 才會被
        fetch_faqs_data() 撈去當正式 FAQ（2026-09 改成白名單制：光填答案還
        不夠，一定要同仁自己把狀態切成「啟用」才會真的上線回覆求職者，避免
        還在編輯中的草稿被誤送出去）。
