...
- `enforce_compliance_rules()` ＝ `enforceComplianceRules()`：兵役、年齡、性別、容貌身家的硬性過濾＋排版標準化，
  AI 寫之前、寫之後各過濾一次。使用者 2026-09-26 同意修掉 GAS 原本的誤刪（「需 8-17點」→「7點」）和漏網
  （「男性優先」、婚姻、籍貫、星座、血型），違法字眼一樣刪、分不清楚時從嚴刪；排版標準化那段沒動。
...
# 2026-09-26 使用者同意修 GAS 原本的誤刪／漏網（HANDOFF「階段 3 第 1 個 PR」）：違反就業服務法的字一樣要刪，
# 分不清楚時從嚴刪；只是不再把「需 8-17點」「須 10-20公斤」「需 2-3 年經驗」這類正常資訊砍掉。
_DIGITS = "[0-9０-９]+|[一二兩三四五六七八九十百]+"
# 數字一定要整段吃完（後面不能再接數字），正規表示式才不會往回退一位繞過單位判斷（GAS 的 bug 就是這樣來的）
_NUM = f"(?:{_DIGITS})(?![0-9０-９一二兩三四五六七八九十百])"
_TO = "[-~～至到約]"
_AGE_UNIT = "(?:歲|周歲|週歲)"
_AGE_SCOPE = "(?:以內|以下|以上|之間|左右|上下|內|前)"
_ASK = "(?:需要|限|須|需|要)"
# 數字範圍後面接這些就是時間、重量、天數、人數、金額…不是年齡，保留
_NOT_AGE_UNITS = ("小時|點|時|分|秒|公斤|kg|KG|Kg|斤|公分|cm|公尺|米|號|樓|段|巷|弄|包|箱|件|年|個|月|天|日|週|周|次|"
                  "人|位|名|元|塊|k|K|度|%|％|趟|台|輛|家|間|班|組|倍|:|：|\\.")
# 年齡合理的範圍；後面什麼單位都沒寫的「限 18-35」只要兩個數字都在這個範圍就當年齡刪
_AGE_MIN, _AGE_MAX = 15, 70
_PLACES = ("台北|臺北|新北|桃園|台中|臺中|台南|臺南|高雄|基隆|新竹|苗栗|彰化|南投|雲林|嘉義|屏東|宜蘭|花蓮|台東|臺東|"
           "澎湖|金門|馬祖|本地|當地|本省|外省")
_ZODIAC = "牡羊|白羊|金牛|雙子|巨蟹|獅子|處女|天秤|天蠍|射手|人馬|摩羯|魔羯|水瓶|雙魚"


def _to_int(value):
    value = str(value).translate(str.maketrans("０１２３４５６７８９", "0123456789"))
    return int(value) if value.isdigit() else None


def _looks_like_age(*values) -> bool:
    """國字數字（「限二十至三十五」）判斷不了大小，從嚴當年齡。"""
    numbers = [_to_int(v) for v in values]
    return all(n is None or _AGE_MIN <= n <= _AGE_MAX for n in numbers)


def _drop_if_age(match) -> str:
    return "" if _looks_like_age(*match.groups()) else match.group(0)
...
def enforce_compliance_rules(text) -> str:
    ...
    # 3. 年齡
    # (A) 有「歲」的範圍：一律刪，連前面的「需／限／年齡」一起刪，不留殘字
    s = re.sub(rf"{_ASK}?\s*(?:年滿|年齡|年紀)?\s*{_NUM}\s*{_TO}\s*{_NUM}\s*{_AGE_UNIT}\s*{_AGE_SCOPE}?", "", s)
    # (B) 前面寫「年齡／年紀」的範圍：一律刪
    s = re.sub(rf"(?:年齡|年紀)\s*{_ASK}?\s*{_NUM}\s*{_TO}\s*{_NUM}(?!\s*(?:{_NOT_AGE_UNITS}))", "", s)
    # (C) 「限／需／須／要／適合＋範圍」後面沒接單位：數字像年齡（15～70）就刪
    s = re.sub(rf"(?:{_ASK}|適合)\s*({_NUM})\s*{_TO}\s*({_NUM})(?!\s*(?:{_NOT_AGE_UNITS}))", _drop_if_age, s)
    # (D) 單一年齡「18歲以上」「須年滿18歲」：一律刪
    s = re.sub(rf"{_ASK}?\s*(?:年滿|年齡|年紀)?\s*{_NUM}\s*{_AGE_UNIT}\s*{_AGE_SCOPE}?", "", s)
    # (E) 「年齡 45 以下」一律刪；「限 45 以下」數字像年齡才刪
    s = re.sub(rf"(?:年齡|年紀)\s*{_ASK}?\s*{_NUM}\s*{_AGE_SCOPE}", "", s)
    s = re.sub(rf"{_ASK}\s*({_NUM})\s*{_AGE_SCOPE}", _drop_if_age, s)
    s = re.sub(r"(限|須|需)?\s*([0-9一二三四五六七八九十]+)年級生", "", s)
    s = re.sub(r"年輕化?的?團隊", "充滿活力的團隊", s)
    s = re.sub(r"年輕有?(?:活力|朝氣)", "充滿活力", s)
    s = re.sub(r"年輕(活力|有幹勁|貌美|力壯)?", "具備熱忱", s)
    s = re.sub(r"年紀(輕|小|大)", "", s)
    # 4. 性別（先換整句，再刪「限男」；長的寫法排前面，才不會留下「性」「生」殘字）
    s = s.replace("限男生搬重", "需配合搬重貨物")
    s = s.replace("限女性細心", "需具備細心度")
    s = re.sub(r"(?:僅限|只限|限)\s*(男性|女性|男生|女生|男士|女士|男|女)", "", s)
    s = re.sub(r"(男性|女性|男生|女生|男士|女士)\s*(?:優先錄取|優先|尤佳|為佳|較佳|佳)", "", s)
    s = re.sub(r"適合(女性|男性|男生|女生)", "歡迎各界人才", s)
    s = s.replace("男女不拘", "歡迎各界人才")
    s = re.sub(r"(男|女)作業員", "作業員", s)
    s = re.sub(r"(男|女)理貨員", "理貨員", s)
    # 5. 身心與容貌、婚姻、籍貫／出生地、星座、血型
    s = re.sub(r"五官端正|容貌端莊|身家清白|無前科|(?:身心|四肢|身體|心智)健全", "", s)
    s = re.sub(r"(?:僅限|只限|限|須|需)\s*(?:未婚|已婚)(?:者|人士)?|(?:未婚|已婚)(?:者|人士)?\s*(?:優先|尤佳|為佳|佳)", "", s)
    s = re.sub(rf"(?:僅限|只限|限|須為|需為)\s*(?:{_PLACES})(?:人|籍)|(?:{_PLACES})(?:人|籍)\s*(?:優先|尤佳|為佳|佳)", "", s)
    s = re.sub(rf"(?:限|須|需|不收|不要)\s*(?:{_ZODIAC})座(?:者)?", "", s)
    s = re.sub(r"(?:限|須|需|不收|不要)\s*(?:AB|A|B|O)\s*型(?:血)?(?:者)?", "", s)
    # 6. 多餘符號與空格
    s = re.sub(r"[,，、]{2,}", "、", s)
    s = re.sub(r"[ \t]+", " ", s)
    s = re.sub(r"^[ \t,，、/／-]+|[ \t,，、/／-]+$", "", s, flags=re.M)
...
# 這些 6 區塊標題改寫成 4 區塊的名稱——這是 GAS 原本就有的行為，照搬。
