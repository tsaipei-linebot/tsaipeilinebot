...
def resolve_apply_url_key_by_industry(job: dict) -> str:
    ...
    分類規則對齊 Notion「小雞上工客服自動化／職缺分類與履歷連結對照表」這份
    同仁維護、確認過的權威對照表：
    - 蝦皮外送一律歸類「服務業」，刻意排在最前面優先判斷——蝦皮外送職缺的
      文字裡同時會出現「蝦皮」跟「外送」，原本的寫法只要看到「蝦皮」就直接
      判定成「蝦皮門市專屬」，導致蝦皮外送誤拿到蝦皮門市版的履歷連結（實測
      回報案例）。
    - 「蝦皮門市專屬」改成只認「蝦皮門市／智取店／店到店／蝦皮店到店／門市
      理貨」這幾個明確組合字，不再用單獨「蝦皮」兩個字判斷——同一份對照表
      裡蝦皮物流、蝦皮外送都不屬於這一類，只看「蝦皮」兩個字沒辦法分辨。
    - 其餘服務業/一般門市關鍵字維持既有覆蓋範圍，另外補上對照表新增的
      「客服」「櫃姐」「櫃哥」「內外場」「門市服務」。
    - 都沒命中時預設「製造業」（維持原本行為）：對照表的製造業關鍵字（作業員、
      產線、包裝、倉管、技術員、科技廠、物流、揀貨、搬運工、理貨、倉儲、
      電商物流、蝦皮物流、momo理貨、pchome理貨）本來就不會被上面兩組關鍵字
      誤判命中，不需要另外寫一次。"""
    full_search_text = f"{job.get('職缺名稱(對外)', '')} {job.get('職缺名稱', '')} {job.get('職務類別', '')} {job.get('行業別', '')} {job.get('工作內容(對外)', '')}".lower()

    if "外送" in full_search_text:
        return "Service"

    if any(k in full_search_text for k in ["蝦皮門市", "智取店", "店到店", "蝦皮店到店", "門市理貨"]):
        return "Spx"

    if any(k in full_search_text for k in ["服務", "餐飲", "服飾", "門市", "專櫃", "店員", "廚助", "客服", "櫃姐", "櫃哥", "內外場", "門市服務"]):
        return "Service"
