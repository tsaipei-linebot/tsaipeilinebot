...
- **獨立訊息系統（2026-09 補充決策）**：長期目標是降低對 LINE 平台的依賴／商業風險，改用完全自建的站內/App 內聊天取代 LINE。時程上不提前到 Phase 0——Phase 0 沿用既有 LINE + Firestore session 模式不變。規劃落點：Phase 2b（開放外部商家）前後開始技術選型設計；Phase 3（規模化，含原生 App）實際建置，搭配推播通知一次到位；Phase 4（越南擴展，LINE 滲透率低、Zalo 為主）成為非做不可的前提。理由見 `job-app-handoff.md` 範圍決策脈絡第6點
...
> 本節已依「V1 內部化」範圍決策（見 `job-app-handoff.md` 範圍決策脈絡）調整，取代原本以開放式雙邊市集為起點的版本。原 Phase 1（開放市集 MVP）由下方 Phase 0 取代。

0. **Phase 0 · 內部化 MVP（現在進行中，Sprint 0-6，樂觀估 10-13 週／務實估 12-16 週，業餘團隊步調）**：`Client.mode=MANAGED` 全流程——StaffUser 操作既有派遣客戶的職缺刊登、應徵者指派、履歷同步。不含金流、公開評價、多租戶自助刊登。
2. **Phase 2 · 開放 SELF_SERVE（約 2 個月，分兩步）**：
   - **2a 內部先行**：自家派遣團隊以正常商家身份（`MerchantUser` 帳號）走真正的自助刊登/審核流程上線，真實求職者可應徵，但免收費、無用量上限，用真實流量驗證自助流程本身
   - **2b 開放外部商家**：驗證過後才開放真外部商家（收費），需搭配評價系統、Vertex AI 語意配對、金流/訂閱上線；**私立就業服務機構許可等法遵確認須在 2b 之前完成**（且 2a 是否也需要確認，見 `job-app-handoff.md` 法遵風險備註）。此時開始獨立訊息系統的技術選型設計，但暫不動工
3. **Phase 3 · 規模化（約 2–3 個月）**：原生 App、ATS 拖拉看板、企業品牌頁、數據儀表板、內容社群、Learning-to-Rank、**獨立訊息系統與推播上線（取代 LINE，與原生 App 一起建置）**
4. **Phase 4 · 擴張**：多語言、跨區域擴展（含越南）、企業 API 開放、與外部 HR 系統整合——**越南版須有獨立訊息系統（LINE 在越南滲透率低），此階段為非做不可的前提**
