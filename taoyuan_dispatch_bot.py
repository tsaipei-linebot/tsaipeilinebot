"""桃園所派遣 LINE 官方帳號的訊息處理邏輯（2026-09-21 新增，桃園所專區
Phase 2）：解析人員傳來的固定指令（綁定/需求列表/報名/我的報名），串接
`services/taoyuan_dispatch_service.py` 做真正的 Firestore 讀寫，回傳要
回覆的文字。`taoyuan_dispatch_webhook_routes.py` 收到文字訊息事件時直接
呼叫 `handle_message()`。
...
邏輯——見 `taoyuan_dispatch_line.py` 開頭說明）。
"""
import re
...
def handle_message(line_user_id: str, text: str) -> str:
    ...
    if cmd_type == CMD_REGISTER:
        posting = service.find_posting_by_short_code(command["short_code"])
        if not posting:
            return "找不到這個代碼對應的需求，請確認代碼是否正確，或傳「需求列表」重新查看。"
        _ok, message = service.register_for_posting(posting["id"], personnel["id"], personnel["name"], line_user_id)
        return message

    if cmd_type == CMD_MY_REGISTRATIONS:
        registrations = service.list_registrations_by_personnel(personnel["id"])
        if not registrations:
            return "您目前沒有任何報名紀錄。"
        lines = [_format_registration_line(r, service.get_posting(r["posting_id"])) for r in registrations]
        return "您的報名紀錄：\n\n" + "\n\n".join(lines)
