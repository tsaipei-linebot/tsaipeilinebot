...
def _is_yes(value: str) -> bool:
    """「待維修」欄認「是」或「有」都算勾選（2026-09-17 起，同仁常把這兩組
    肯定詞混著填，跟意外事件回報的「是否報警」/「是否聯繫家屬」一樣統一
    當同義詞接受），其餘（留空、填「否」/「無」、打錯字）一律當作沒有勾選，
    不會擋下整筆回報——比起嚴格擋錯，讓同仁「沒填清楚就當作沒事」風險更低
    （車輛正常，只是差了一個待維修的標記，管理員之後在網頁上還是能補標記）。"""
    return (value or "").strip() in ("是", "有")
