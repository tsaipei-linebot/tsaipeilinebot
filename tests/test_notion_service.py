...
class LooksUnsuitableForFaqCandidateTests(unittest.TestCase):
    """使用者實測回報：FAQ 候選清單（每週推播到 LINE 群組的報告會原樣列出來）
    裡混進了求職者姓名+電話（例如確認履歷用）、廣告/詐騙連結，這些內容不該
    被記錄進候選清單。"""

    def test_phone_number_is_unsuitable(self):
        self.assertTrue(n._looks_unsuitable_for_faq_candidate("蔡旻玲，0977557839"))
        self.assertTrue(n._looks_unsuitable_for_faq_candidate("好了👌王松博0905038083 方案2"))

    def test_url_is_unsuitable(self):
        self.assertTrue(n._looks_unsuitable_for_faq_candidate("點我看更多 https://shp.ee/zlfaywdctgi"))

    def test_overly_long_text_is_unsuitable(self):
        self.assertTrue(n._looks_unsuitable_for_faq_candidate("麻豆老欉文旦" * 30))

    def test_normal_short_question_is_suitable(self):
        self.assertFalse(n._looks_unsuitable_for_faq_candidate("發薪日是什麼時候"))
        self.assertFalse(n._looks_unsuitable_for_faq_candidate("加班費怎麼計算？"))
        self.assertFalse(n._looks_unsuitable_for_faq_candidate("請問可以先幫我取消今天的面試嗎？非常不好意思"))

    def test_empty_text_is_unsuitable(self):
        self.assertTrue(n._looks_unsuitable_for_faq_candidate(""))


class AppendUnresolvedFaqFiltersUnsuitableContentTests(unittest.TestCase):
    """驗證過濾邏輯確實接在 append_unresolved_faq_to_notion() 前面生效，且
    完全不影響 append_unresolved_question_for_followup()——後者是招募專員
    用來回頭找到求職者本人的紀錄，即使內容含姓名/電話也必須完整保留。"""
    ...
    def test_phone_number_content_is_not_written(self):
        with patch("services.notion_service.NOTION_API_KEY", "dummy-key"), \
             patch("services.notion_service.NOTION_FAQ_DB_ID", "dummy-db-id"), \
             patch("services.notion_service.requests.post") as mock_post:
            result = n.append_unresolved_faq_to_notion("蔡旻玲，0977557839")

        self.assertFalse(result)
        mock_post.assert_not_called()

    def test_normal_question_is_still_written(self):
        mock_response = type("_Resp", (), {"status_code": 201, "text": ""})()
        with patch("services.notion_service.query_notion_database_direct", return_value=[]), \
             patch("services.notion_service.NOTION_API_KEY", "dummy-key"), \
             patch("services.notion_service.NOTION_FAQ_DB_ID", "dummy-db-id"), \
             patch("services.notion_service.requests.post", return_value=mock_response) as mock_post:
            result = n.append_unresolved_faq_to_notion("颱風天上班算加班嗎")

        self.assertTrue(result)
        mock_post.assert_called_once()

    def test_followup_tracking_is_unaffected_by_faq_candidate_filter(self):
        # 求職者告知姓名+電話確認履歷，這種內容必須完整寫進求職者提問追蹤，
        # 不能因為 FAQ 候選清單那邊會過濾，就連這裡也被連帶擋掉。
        mock_response = type("_Resp", (), {"status_code": 201, "text": ""})()
        with patch("services.notion_service.NOTION_API_KEY", "dummy-key"), \
             patch("services.notion_service.NOTION_UNRESOLVED_QUESTIONS_DB_ID", "dummy-db-id"), \
             patch("services.notion_service.requests.post", return_value=mock_response) as mock_post:
            result = n.append_unresolved_question_for_followup(
                "我叫蔡旻玲，電話0977557839，麻煩確認履歷", "U1234", display_name="蔡旻玲"
            )

        self.assertTrue(result)
        _, kwargs = mock_post.call_args
        self.assertEqual(
            kwargs["json"]["properties"]["提問內容"]["rich_text"][0]["text"]["content"],
            "我叫蔡旻玲，電話0977557839，麻煩確認履歷",
        )
