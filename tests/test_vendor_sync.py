...
class SyncVendorFromClientContractTests(unittest.TestCase):
    ...
    def test_always_creates_a_new_vendor_without_checking_duplicates(self):
        with mock.patch.object(vendor_sync.platform_vendors, "create_vendor_auto", return_value="new-id") as mock_create:
            with mock.patch.object(vendor_sync.platform_vendors, "find_vendor_id_by_name") as mock_find:
                result = vendor_sync.sync_vendor_from_client_contract(
...
class SyncVendorFromDispatchContractTests(unittest.TestCase):
    """派遣契約產生器送出成功後的同步邏輯：廠商管理裡已經有同名紀錄就
    沿用那一筆的 ID，沒有才新增一筆，避免同一個客戶重複產生很多份派遣
    契約時灌爆重複紀錄。"""

    def test_creates_vendor_when_name_not_already_present(self):
        with mock.patch.object(vendor_sync.platform_vendors, "find_vendor_id_by_name", return_value=None):
            with mock.patch.object(vendor_sync.platform_vendors, "create_vendor_auto", return_value="new-id") as mock_create:
                result = vendor_sync.sync_vendor_from_dispatch_contract("新客戶")
        mock_create.assert_called_once_with({"name": "新客戶"})
        self.assertEqual(result, "new-id")

    def test_reuses_existing_vendor_id_when_name_already_present(self):
        with mock.patch.object(vendor_sync.platform_vendors, "find_vendor_id_by_name", return_value="existing-id"):
            with mock.patch.object(vendor_sync.platform_vendors, "create_vendor_auto") as mock_create:
                result = vendor_sync.sync_vendor_from_dispatch_contract("既有客戶")
    ...
    def test_blank_name_is_ignored(self):
        with mock.patch.object(vendor_sync.platform_vendors, "find_vendor_id_by_name") as mock_find:
            with mock.patch.object(vendor_sync.platform_vendors, "create_vendor_auto") as mock_create:
                result = vendor_sync.sync_vendor_from_dispatch_contract("   ")
        mock_find.assert_not_called()
        mock_create.assert_not_called()
