...
from hr.incident_report import handle_incident_report, parse_incident_report
from hr.repository import incident_matches_filters
...
class ParseIncidentReportTests(unittest.TestCase):
    def test_valid_message_parses(self):
        ...
        # 廠商名稱是自由文字，不像配送部那套需要對照白名單。
        self.assertEqual(result["vendor"], "XX工廠")
    ...
    def test_free_text_vendor_is_not_validated(self):
        # 這是跟配送部那套最大的差異：廠商名稱隨便填什麼都會通過解析。
        text = _replace_field(_VALID_TEXT, "1.廠商名稱", "1.廠商名稱：隨便填的名稱")
        result = parse_incident_report(text)
        self.assertTrue(result["ok"], result)
        self.assertEqual(result["vendor"], "隨便填的名稱")
    ...
    @patch("hr.repository.create_incident_event")
