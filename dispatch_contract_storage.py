...
def delete_file(blob_path: str):
    """契約紀錄被刪除時一併清掉存好的檔案——跟 client_contract_storage.py
    的 delete_file() 同一種寫法：`blob_path` 空字串（例如當初 PDF 轉檔
    失敗、根本沒存過）或檔案本來就不存在都安靜跳過，不算錯誤，呼叫端
    （刪除整筆紀錄）不應該因為這裡失敗就整個中斷。"""
    if not blob_path or not blob_path.startswith("dispatch_contracts/"):
        return
    blob = _bucket().blob(blob_path)
    if blob.exists():
        blob.delete()
