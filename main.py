...
1. 階段一【初次對話 / 深入了解 (INTENT: CHAT)】：
   - 若為對話剛開始、打招呼、或求職者的條件「尚未完整」（例如只提了地區，但班別或工作內容還不知道）：
   - 必須設定 INTENT 為 CHAT，IDS 填寫空字串。
   - 若對話剛開始或求職者只簡單問候，請以最親切專業的招募專員口吻引導詢問工作地區與班別需求。
   - 若求職者已講出部分條件（如地區），請先肯定確認，並【自然追問下一個未確認的重點（例如班別、工作性質或薪資需求）】。
   - 提供 3-5 個適合的 QuickReply 按鈕（例如：☀️ 固定早班, 🌙 夜班, 📦 物流理貨, 🏭 廠區作業員）。

2. 階段二【精準推薦 (INTENT: RECOMMEND)】：
   - 只要符合下列任一情況，請務必設定 INTENT 為 RECOMMEND：
     (a) 求職者已確認【地區】＋【班別】或【工種】（例如已得知新莊 + 早班）。
     (b) 求職者明確要求看職缺（例如「查看職缺細節」、「查看詳細簡章」、「適合我的工作」、「推薦給我」、「有哪些工作」）。
...
"""
    ...
    # 顯性求職觸發詞偵測 (求職者主動點按鈕要求看卡片時，強制 RECOMMEND)
    recommend_triggers = [
        "適合", "職缺", "簡章", "工作", "有哪些", 
        "推薦", "看卡片", "清單", "我想看", "細節", "找工作"
    ]
    if any(k in raw_msg for k in recommend_triggers) and user_profile.get("area"):
        intent = "RECOMMEND"

    if any(kw in reply_text for kw in ["下方職缺卡片", "下方卡片", "點擊下方查看", "詳細簡章"]) and user_profile.get("area"):
        intent = "RECOMMEND"
    ...
    # 6. 依意圖發送訊息與職缺卡片 (只有 RECOMMEND 且有符合職缺時才發送 Flex 卡片)
    if intent == "RECOMMEND":
